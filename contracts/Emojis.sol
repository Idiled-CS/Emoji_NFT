...
contract Emojis is ERC721Enumerable, Ownable {
    ...
    string private svgBeginning =
        '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 -0.5 64 64" shape-rendering="crispEdges">';
    ...
    function createSVG() internal view returns (string memory) {
        string memory _svgOutput = string(
            abi.encodePacked(
                svgBeginning,
    ...
    function createTraitsJSON() internal view returns (string memory) {
        string memory _traitsOutput = string(
            abi.encodePacked(
                '"attributes":',
                "[",
                '{"trait_type":"Face"',
                '"value":',
                _layer1Names[_emoObj.Layer1],
                "}",
                '{"trait_type":"Eyes"',
                '"value":',
                _layer2Names[_emoObj.Layer2],
                "}",
                '{"trait_type":"Mouth"',
                '"value":',
                _layer3Names[_emoObj.Layer3],
                "}",
                '{"trait_type":"Accessories"',
                '"value":',
                _layer4Names[_emoObj.Layer4],
                "}",
                "]"
            )
        );
        ...
        returns (string memory)
    {
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "On-Chain Emojis #',
                        toString(tokenId),
                        '", "description": "What emoji ya got?"',
                        createTraitsJSON(),
                        '"image": "data:image/svg+xml;base64,',
                        Base64.encode(bytes(createSVG())),
                        '"}'
                    )
                )
            )
    ...
    function mint(address wallet, uint256 num) public payable {
        ...
        require(
            (_totalMinted + num) <= _maxSupply,
            "Sales ended or you tried to mint over the supply"
        );
